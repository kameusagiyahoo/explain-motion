...
export function ExplainVideo({ plan }: { plan: VideoPlan }) {
  ...
  return <AbsoluteFill><TransitionSeries>{plan.scenes.flatMap((scene, index) => {
  ...
  })}</TransitionSeries>{isVideoPlanV2(plan) && <CaptionOverlay plan={plan} theme={themes[plan.style]} />}</AbsoluteFill>;
}
